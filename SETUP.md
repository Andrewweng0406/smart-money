...
綜合摘要會先顯示每檔的決策姿態、信心與上下觸發條件，再列原始籌碼價位。
資料品質低或重大事件臨近時一律降為低信心觀望；正常資料也暫時只給中信心，
直到歷史訊號審核累積足夠樣本，不把規則式判斷包裝成已驗證的方向預測。
...
只有在 NYSE 行事曆認定的美股股票期權正式交易時間才會執行檢查：一般交易日
是 09:30~16:00 美東，完整休市日不執行，半日市則依實際時間提前停止。盤前、
盤後與休市時段不會浪費 API 呼叫，也不會發出無法交易的期權訊號。
