...
跟 gex_engine.py 一樣只吃數字/dict 進來、回傳 dict 出去，方便單獨測試。放在
獨立檔案而不是塞進 gex_engine.py，是因為這裡做的是「dealer 部位 + Wall +
Gamma 狀態」三者合起來的規則型判斷（跟 smart_money.py 的
compute_market_maker_pressure_score 屬於同一類：不是 Black-Scholes 算式本身，
是在已經算出來的 Greeks/OI 結果之上做解讀）。
...
現貨反而容易順著原本的方向加速——這正是 gex_engine.py find_gamma_flip_point
判斷的正負 Gamma 分水嶺在這裡被拿來當「Pinning 是否成立」的必要條件。
"""
...
# 只是「還沒被磁吸到」而已。跟 smart_money.py 的
# UNUSUAL_ACTIVITY_MAX_DISTANCE_PCT 一樣是經驗法則、非回測驗證過的最佳解。
...
# find_pin_strike 找候選履約價時，只看現貨這個距離百分比以內的——跟
# analyze.py 的 UNUSUAL_ACTIVITY_MAX_DISTANCE_PCT 同一套防禦邏輯：彙總
# 多個到期日後，遠離現貨的稀疏舊倉位（LEAPS留下的巨量OI）常常單一履約價
# 的絕對OI就大過所有近端履約價總和，會把 Pin Strike 誤導到跟「現貨真的
# 會被磁吸到哪裡」完全無關的地方。這是實測抓到的真bug：真實 TSLA 資料
# 曾經測出 Pin Strike 落在距現貨 21.7% 遠的履約價，明顯是LEAPS的陳舊
# 大倉位，不是近端的 Pinning 磁吸點。
PIN_CANDIDATE_MAX_DISTANCE_PCT = 20.0

# 0～100 分數換算成文字標籤的門檻，跟 smart_money.py
# compute_market_maker_pressure_score 用同一組門檻，維持全專案「分數轉
# 標籤」的判讀慣例一致。
...
def find_pin_strike(gex_by_strike: Sequence[dict], spot: float | None = None) -> float | None:
    ...
    跟 Max Pain（讓期權買方到期內在價值總和最小的履約價，analyze.py 的
    _calculate_max_pain）是兩個不同的概念，經常但不總是重合：Max Pain 是
    「對期權賣方最有利」的理論價位，Pin Strike 是「未平倉量最集中、做市商
    避險部位最大」的實際價位——後者才是 Gamma Pinning 效應真正的物理
    機制（做市商為了維持 Delta 中性，在這個履約價附近的避險買賣盤最密集）。
    呼叫端可以同時顯示兩者、標示是否重合，重合時代表訊號更一致。
...
def score_pinning(
    ...
    的透明分數（三個分量，跟 smart_money.py
    compute_market_maker_pressure_score 同樣的設計慣例：不是回測驗證過的
    最佳解，是把判斷邏輯攤開透明，而不是黑箱模型）。
    ...
    Regime 三選一：
      ...
      - PINNING：現貨在 Wall 區間內、且分數達到「高」以上（>=60，跟
        compute_market_maker_pressure_score 的標籤門檻一致）。
    ...
    # 加 0.5 取整以符合一般「四捨五入」而非銀行家捨入，跟
    # compute_market_maker_pressure_score 同樣的寫法。
